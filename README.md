...
### One-Click Startup
From your terminal, run the orchestrator script in the project root directory:
```powershell
python run.py
```
...
To execute backend verification checks, run:
```powershell
cd backend
python -m pytest
```
...
## 🛡️ Scalability & Integration Narrative
- **Phase 1**: Pilot integration with **SEBI SCORES** portal as a pre-screening automated layer to parse evidence logs and file formatted grievance dossiers.
